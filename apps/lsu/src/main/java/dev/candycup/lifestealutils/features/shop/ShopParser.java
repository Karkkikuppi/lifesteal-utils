...
public final class ShopParser {
    ...
    public static ShopParseResult parseListings(String title, String categoryName, List<SlotSnapshot> shopItems) {
        ...
        for (SlotSnapshot item : shopItems) {
            ...
            listings.add(new ShopView.ListingRow(listingName(item), meta.coinsPer(), meta.xpPer(), item.slotIndex(), item));
        }
    ...
    private static String listingName(SlotSnapshot item) {
        if (item.stack().getItem() != Items.ENCHANTED_BOOK || item.stack().has(DataComponents.CUSTOM_NAME)) {
            return item.plainName();
        }

        ItemEnchantments enchantments = item.stack().get(DataComponents.STORED_ENCHANTMENTS);
        if (enchantments == null || enchantments.isEmpty()) {
            return item.plainName();
        }

        // A book can technically store multiple enchantments, so retain all useful identity instead of picking one arbitrarily.
        List<String> names = new ArrayList<>(enchantments.size());
        for (var enchantment : enchantments.entrySet()) {
            names.add(Enchantment.getFullname(enchantment.getKey(), enchantment.getIntValue()).getString());
        }
